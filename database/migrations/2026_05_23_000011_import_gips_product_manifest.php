...
return new class extends Migration {
    public function up(): void
    {
        ...
        app(GipsProductManifestImporter::class)->import(base_path('produkter/produkter_manifest.csv'));
        ...
        DB::table('order_items')
            ...
            ->chunkById(200, function ($items) use ($resolver) {
                foreach ($items as $item) {
                    $product = $resolver->forArticle(($item->product_sku ?? null) ?: $item->artikel);
                    if (! $product) {
                        continue;
                    }
