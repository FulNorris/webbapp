...
return new class extends Migration {
    ...
    private function importProductManifest(): void
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $title = trim((string) ($data['title'] ?? $sku));
            $products[$sku] ??= [
                'sku' => $sku,
                'folder' => $folder,
                'title' => $title,
            ...
            $products[$sku]['image_count']++;

            $images[] = [
                'product_sku' => $sku,
                'image_index' => $products[$sku]['image_count'],
                'folder' => $folder,
                'image_path' => $imagePath,
                'image_url' => $this->imageUrl($imagePath),
                'source_page' => trim((string) ($data['source_page'] ?? '')) ?: null,
                'source_image' => trim((string) ($data['source_image'] ?? '')) ?: null,
                'title' => $title,
                'created_at' => now(),
                'updated_at' => now(),
            ];
